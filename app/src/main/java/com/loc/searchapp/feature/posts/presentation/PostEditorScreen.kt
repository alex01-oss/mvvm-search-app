...
import com.loc.searchapp.feature.posts.components.EditorTopBar
import com.loc.searchapp.feature.posts.components.ErrorDialog
import com.loc.searchapp.feature.posts.components.PostImagePicker
import com.loc.searchapp.feature.posts.components.RichTextToolbar
...
fun PostEditorScreen(
    ...
    LaunchedEffect(post) {
        post?.let { richTextState.setHtml(it.content) }
    }
    ...
    val onSaveClick = {
        ...
        fun savePost(finalImageUrl: String?) {
            if (post == null) {
                ...
                viewModel.editPost(post.id.toInt(), formState.title, content, finalImageUrl ?: "")
            }
            isLoading = false
            onFinish()
        }
